const HEDGE_PATTERN =
  /\b(not sure|unsure|don't have|do not have|don't know|do not know|need more|which one|could you clarify|clarify|what do you mean|assuming|i'll assume|i will assume|no record|not aware|couldn't find|could not find|not familiar|doesn't appear to exist|does not appear to exist|seems (off|wrong|unusual|unlikely)|that (doesn't|does not) (look|seem) right|double[- ]check|double check|verify|inconsistent(?:ly)?|conflicts?|conflicting|contradicts?|contradicting|contradictory|discrepanc(?:y|ies)|unclear|ambiguous)\b/i;
